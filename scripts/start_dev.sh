...
if [ "$(backend_http)" = "200" ]; then
  ...
  echo "       backend PID=$!（日志: tail -f /tmp/uvicorn.log）"
fi
...
if [ "$(front_http)" = "200" ]; then
  ...
  echo "       vite PID=$!（日志: tail -f /tmp/vite.log）"
fi
...
echo "  [tips] 上方两个服务确认 HTTP 200 后再打开浏览器"
